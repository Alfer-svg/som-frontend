...
document.addEventListener('alpine:init', () => {
  Alpine.data('app', () => ({
    ...
    async carregarLeads() { try { const r = await this.api('GET', '/leads'); let ls = (r || []).map(x => ({ id: x.id, ...(x.dados || {}) })); if (!this.ehAdmin) { const eu = (this.usuario && this.usuario.nome) || ''; ls = ls.filter(l => (l.responsavel || '') === eu); } this.leads = ls; } catch { } },
    ...
    go(p) { if (!this.podeVer(p)) return; this.menuAberto = false; this.page = p; MD.set('som_page', p); this.busca = ''; if (p === 'monitoramento' && this.monitorCliente) this.carregarCredenciais(this.monitorCliente.id); if (p === 'comercial') { this.comTab = 'lista'; this.carregarOnboardings(); } if (p === 'crm') { this.carregarLeads(); this.carregarCrmStages(); if (this.ehAdmin) this.carregarUsuarios(); } if (p === 'dashboard') { if (!this.ehAdmin) this.dashTab = 'comercial'; this.carregarCrmStages(); this.carregarPropostas(); this.carregarMetas(); this.carregarLeads().then(() => { if (this.page === 'dashboard' && this.dashTab === 'comercial' && this.motivacao) this.mostrarToast(this.motivacaoMsg); }); } if (p === 'pessoal') { this.carregarUsuarios(); } if (p === 'configuracoes') { this.carregarUsuarios(); this.carregarCloud(); this.carregarPapeis(); this.carregarMetaApp(); this.carregarMetaStatus('maracatu'); } if (p === 'operacional') { this.versiculo = this.sorteiaVersiculo(); if (this.papel === 'colaborador2') this.opTab = 'quadro'; this.ajustaAbaOperacional(); this.carregarClientes(); this.carregarPresenca(); this.carregarProjetos(); this.carregarLayouts(); this.carregarLabels(); this.carregarBoards(); this.carregarCloud(); this.carregarOperacoes(); this.carregarSamara(); } if (p === 'infostatus') { this.infoAba = this.ehAdmin ? 'status' : 'senhas'; if (this.ehAdmin) this.carregarInfoStatus(); this.carregarCofreSenhas(); } if (p === 'relatorios') this.carregarRelatorio(); if (p === 'trafego') this.carregarTrafego(); if (p === 'candidato') { (async () => { if (!(this.clients || []).length) await this.carregarClientes(); const cs = this.candClientes(); if (!cs.length) { this.page = this.paginaInicial; return; } const salvo = localStorage.getItem('som_cand_sel'); this.candSel = (salvo && cs.some(c => c.id === salvo)) ? salvo : cs[0].id; })(); } if (p === 'areacliente') { (async () => { if (!(this.clients || []).length) await this.carregarClientes(); const cs = this.acClientes(); if (!cs.length) { this.page = this.paginaInicial; return; } const salvo = localStorage.getItem('som_ac_sel'); this.acSel = (salvo && cs.some(c => c.id === salvo)) ? salvo : cs[0].id; })(); } if (p === 'operacoes') { this.operTab = 'painel'; this.carregarClientes(); this.carregarProjetos(); this.carregarEquipe(); this.carregarPresenca(); this.carregarSamara(); this.carregarOperacoes(); if (!this.matTop) this.carregarMelhores(); } },
    ...
    get agEventos() {
      ...
      for (const c of (this.clients || [])) {
        ...
        for (const e of (c.agenda || [])) { if (e && e.data) out.push({ data: String(e.data).slice(0, 10), hora: e.hora || '', tipo: e.tipo || 'Evento', titulo: e.titulo || e.tipo || 'Evento', nota: e.nota || '', link: e.link || '', membros: Array.isArray(e.membros) ? e.membros : [], desenvolvimento: e.desenvolvimento || '', solucao: e.solucao || '', satisfacao: +e.satisfacao || 0, satisfacaoJust: e.satisfacaoJust || '', clienteId: c.id, cliente: nome, fonte: 'agenda', id: e.id }); }
    ...
    async carregarOperacoes() {
      ...
      try {
        ...
        this.operChecklists = (Array.isArray(cks) ? cks : []).filter(x => x && x.data && (x.clienteId || x.pessoa));
  ...
  <div class="rodape"><hr class="regua" style="margin-bottom:0"><div class="txt">${esp('MARACATU DIGITAL INTELLIGENCE')}</div></div>
    ...
    trafFichDo(dia) {
      ...
      const g = (nome, clienteId) => {
        ...
        if (!groups[k]) groups[k] = { cliente: k, clienteId: clienteId || '', trafDoc: null, trafLog: [], posts: [], operLog: [], rotina: null };
    ...
    verCandidato(id) { this.candModal = false; this.candSel = id; this.page = 'candidato'; MD.set('som_page', 'candidato'); localStorage.setItem('som_cand_sel', id); }, // persiste: recarregar volta AQUI (antes voltava pra última página que passou pelo go())
    // Área do Cliente (painel-cliente) DENTRO do SOM — mesmo padrão do Candidato,
    // mas vale pra TODOS os clientes: sem modal, escolhe no <select> do cabeçalho.
    acSel: '', // clienteId aberto na Área do Cliente (iframe)
    acClientes() { return (this.clients || []).filter(c => c.status !== 'Inativo').sort((a, b) => (a.empresa || a.nome || '').localeCompare(b.empresa || b.nome || '', 'pt')); },
    _trafRelRange() {
    ...
    async salvarEvento() {
      ...
      if (+f.satisfacao && !(f.satisfacaoJust || '').trim()) return alert('Justifique a satisfação do cliente — por que essa carinha?');
      ...
      if (f.id) { const i = agenda.findIndex(e => e.id === f.id); if (i >= 0) agenda[i] = { ...agenda[i], data: f.data, hora: f.hora || '', tipo: f.tipo, titulo: f.titulo.trim(), nota: f.nota, link: (f.link || '').trim(), membros: f.membros || [], desenvolvimento: (f.desenvolvimento || '').trim(), solucao: (f.solucao || '').trim(), satisfacao: +f.satisfacao || 0, satisfacaoJust: (f.satisfacaoJust || '').trim() }; }
      else agenda.push({ id: MD.uid(), data: f.data, hora: f.hora || '', tipo: f.tipo, titulo: f.titulo.trim(), nota: f.nota || '', link: (f.link || '').trim(), membros: f.membros || [], desenvolvimento: (f.desenvolvimento || '').trim(), solucao: (f.solucao || '').trim(), satisfacao: +f.satisfacao || 0, satisfacaoJust: (f.satisfacaoJust || '').trim(), por: (this.usuario && this.usuario.nome) || '', em: new Date().toISOString() });
...
<div class="foot">${e(EMPRESA.nome)} · CNPJ ${e(EMPRESA.cnpj)} · ${e(EMPRESA.endereco)}</div>
    ...
    novoLead(stage) { stage = stage || ((this.crmStages || [])[0] || {}).id || 'Novo'; this.editing = { id: '', empresa: '', contato: '', whatsapp: '', email: '', cidade: '', servico: '', origem: 'Instagram', cnpj: '', valor: 0, stage, notas: '', createdAt: MD.today(), responsavel: (this.usuario && this.usuario.nome) || '' }; this.cnpjMsg = ''; this.modal = 'lead'; },
    ...
    verPainelCliente(c) {
      ...
      window.open('painel-cliente.html?cliente=' + encodeURIComponent(id) + '&v=13', '_blank');
    },
...
${this._docFoot()}
    ...
    async uploadDocumento(e, d) {
      const file = e.target.files && e.target.files[0]; if (!file) return;
      if (!this.cloudOk) await this.carregarCloud(); // a tela de editar cliente não carrega o cloud sozinha
      if (!this.cloudOk) { alert('Configure o Cloudinary primeiro (Pessoal › Armazenamento de arquivos).'); e.target.value = ''; return; }
      this.uploadando = true;
      try {
        const fd = new FormData(); fd.append('file', file); fd.append('upload_preset', this.cloudCfg.preset);
        ...
        if (j.secure_url) { d.url = j.secure_url; if (!(d.nome || '').trim()) d.nome = file.name; }
      } catch (err) { alert(err.message); } finally { this.uploadando = false; e.target.value = ''; }
    },
    ...
    // (criado na 1ª vez, com paleta roxa), atribuído a quem tem o perfil gestortrafego e
    // já com o checklist das demandas da função. UMA janela só: abre direto o editor do
    ...
    async novoTrafego() {
      ...
      const DEMANDAS = [
        'Alinhar objetivo e verba com o cliente',
        'Configurar/verificar pixel e eventos de conversão',
        'Definir públicos e segmentação',
        'Solicitar/subir criativos e copies',
        'Montar campanha e definir orçamento diário',
        'Publicar e conferir aprovação dos anúncios',
        'Acompanhar CPA/CPL dos primeiros dias',
        'Otimizar: pausar ruins, escalar vencedores',

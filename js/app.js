...
document.addEventListener('alpine:init', () => {
  Alpine.data('app', () => ({
    ...
    eventoModal: false,
    eventoForm: { id: '', clienteId: '', data: '', hora: '', tipo: 'Reunião', titulo: '', nota: '', link: '', membros: [], desenvolvimento: '', solucao: '', satisfacao: 0 },
    ...
    get agEventos() {
      ...
      for (const c of (this.clients || [])) {
        ...
        for (const e of (c.agenda || [])) { if (e && e.data) out.push({ data: String(e.data).slice(0, 10), hora: e.hora || '', tipo: e.tipo || 'Evento', titulo: e.titulo || e.tipo || 'Evento', nota: e.nota || '', link: e.link || '', membros: Array.isArray(e.membros) ? e.membros : [], desenvolvimento: e.desenvolvimento || '', solucao: e.solucao || '', satisfacao: +e.satisfacao || 0, clienteId: c.id, cliente: nome, fonte: 'agenda', id: e.id }); }
    ...
    novoEvento(dia) { if (!this.ehAdmin && this.papel !== 'gestortrafego') return; this.eventoForm = { id: '', clienteId: this.agendaFiltro || '', data: dia || this.agendaDiaSel || this._hojeStr(), hora: '', tipo: 'Reunião', titulo: '', nota: '', link: '', membros: [], desenvolvimento: '', solucao: '', satisfacao: 0 }; this.eventoModal = true; },
    editarEvento(ev) { if (ev.fonte !== 'agenda') return; this.eventoForm = { id: ev.id, clienteId: ev.clienteId, data: ev.data, hora: ev.hora || '', tipo: ev.tipo, titulo: ev.titulo, nota: ev.nota || '', link: ev.link || '', membros: Array.isArray(ev.membros) ? ev.membros.slice() : [], desenvolvimento: ev.desenvolvimento || '', solucao: ev.solucao || '', satisfacao: +ev.satisfacao || 0 }; this.eventoModal = true; },
    ...
    async salvarEvento() {
      ...
      if (f.id) { const i = agenda.findIndex(e => e.id === f.id); if (i >= 0) agenda[i] = { ...agenda[i], data: f.data, hora: f.hora || '', tipo: f.tipo, titulo: f.titulo.trim(), nota: f.nota, link: (f.link || '').trim(), membros: f.membros || [], desenvolvimento: (f.desenvolvimento || '').trim(), solucao: (f.solucao || '').trim(), satisfacao: +f.satisfacao || 0 }; }
      else agenda.push({ id: MD.uid(), data: f.data, hora: f.hora || '', tipo: f.tipo, titulo: f.titulo.trim(), nota: f.nota || '', link: (f.link || '').trim(), membros: f.membros || [], desenvolvimento: (f.desenvolvimento || '').trim(), solucao: (f.solucao || '').trim(), satisfacao: +f.satisfacao || 0, por: (this.usuario && this.usuario.nome) || '', em: new Date().toISOString() });
